...
- [`mesh_connected_components`](mesh.md#navis_fastcore.mesh_connected_components): connected components of a triangle mesh, joined at the corners (`connectivity="vertex"`), across any shared edge (`"face"`), or only across edges carrying exactly two faces (`"manifold"`)
